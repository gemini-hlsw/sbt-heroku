...
class SbtApp(
  ...
  fatjar:        Option[File],
  log:           Logger
) extends App(buildPackDesc, if (name.isEmpty) null else name, rootDir, targetDir, buildpacks) {
  ...
  override def logUploadProgress(uploaded: java.lang.Long, contentLength: java.lang.Long) {
    ...
    if (percentUpload != newPercent) {
      ...
      log.info(
        "\u001B[A\r\u001B[2K" + s"[${Level.Info.toString}] -----> Uploading slug... ($percentUpload%)"
      )
    }
  }
  ...
  override def deploy(
    includedFiles: java.util.List[java.io.File],
    configVars:    java.util.Map[String, String],
    jdkVersion:    String,
      ...
      if ((getRootDir / "Procfile").exists() || !processTypes.isEmpty)
        ...
        packageType match {
          case Universal(dir)   =>
            ...
            if (
              scala.io.Source
                .fromFile(targetDir / "/universal/stage/bin" / startScript)
                .mkString
                .contains("-main")
            ) {
              Map[String, String](
      ...
      case e: HttpResponseException =>
        if (e.getStatusCode == 404) {
          throw new RuntimeException(
            s"Could not find app '$name'. Check that herokuAppName setting is correct."
          )
        } else if (e.getStatusCode == 403 || e.getStatusCode == 401) {
          throw new RuntimeException(
            "Check that herokuAppName name is correct. If it is, check that HEROKU_API_KEY is correct or if the Heroku CLI is installed."
          )
        }
        throw e
    }
  ...
  override def prepare(
    ...
    sbt.IO.copyDirectory(targetDir / "resolution-cache" / "reports",
                         getAppDir / "target" / "resolution-cache" / "reports"
    )
